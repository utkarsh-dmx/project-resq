...
class LlamaDecoderLayer(nn.Module):
    def __init__(self, config: LlamaConfig, layer_idx: int):
        ...
        self.basis_change_1 = QuantizeLinear(
            self.hidden_size, self.hidden_size, bias=False
        )
        self.basis_change_2 = QuantizeLinear(
            self.hidden_size, self.hidden_size, bias=False
        )
    ...
    def forward(
        ...
        # Self Attention
        hidden_states, self_attn_weights, present_key_value, inf_norm_attn = (
            self.self_attn(
                hidden_states=hidden_states,
                attention_mask=attention_mask,
                position_ids=position_ids,
...
#     p1 = (eye - sk_sm).to(torch.float64)
#     p2 = torch.inverse((eye + sk_sm).to(torch.float64))
...
class LlamaForCausalLM(LlamaPreTrainedModel):
    ...
    def forward(
        ...
        if labels is not None:
            ...
            # loss = loss_fct(shift_logits, shift_labels) + 0.05 * model_kurtoses
            # loss = loss_fct(shift_logits, shift_labels) + 0.5 * (model_kurtoses) ** 2
            # loss = loss_fct(shift_logits, shift_labels) + 10.0 * model_inf_norm
