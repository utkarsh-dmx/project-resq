...
class ActQuantWrapper(torch.nn.Module):
    ...
    def forward(self, x, pre_rotation=None, post_rotation=None):
        x_dtype = x.dtype
        # Rotate, if needed
        ...
        if self.quantizer.bits < 16:  # Quantize, if needed
            ...
            self.quantizer.free()
        if pre_rotation is not None or post_rotation is not None:
            x = self.module(
...
class RotLinear(torch.nn.Linear):
    def __init__(
        ...
        self.fwd_dunc = torch.nn.functional.linear

        self.pre_rotation = None
        self.post_rotation = None

    def forward(self, input: torch.Tensor, pre_rotation=None, post_rotation=None):
        # inputs to the linear layer is already assumed to be rotated.
        weight = self.weight
        bias = self.bias
