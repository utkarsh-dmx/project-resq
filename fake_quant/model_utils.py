...
LLAMA_MODEL_ROT = modeling_llama.LlamaForCausalLM
LLAMA_LAYER = transformers.models.llama.modeling_llama.LlamaDecoderLayer
LLAMA_LAYER_ROT = modeling_llama.LlamaDecoderLayer
...
def model_type_extractor(model):
    if isinstance(model, LLAMA_MODEL):
        ...
        return OPT_MODEL
    elif isinstance(model, LLAMA_MODEL_ROT):
        return LLAMA_MODEL_ROT
    else:
...
def get_rope_function_name(model):
    if isinstance(model, LLAMA_MODEL) or isinstance(model, LLAMA_MODEL_ROT):
...
def get_embeddings(model, model_type) -> list[torch.nn.Module]:
    ...
    elif model_type == LLAMA_MODEL_ROT:
        return [model.model.embed_tokens]
...
def get_transformer_layers(model, model_type):
    if model_type == LLAMA_MODEL or model_type == LLAMA_MODEL_ROT:
...
def get_pre_head_layernorm(model, model_type):
    if model_type == LLAMA_MODEL or model_type == LLAMA_MODEL_ROT:
        ...
        ) or isinstance(pre_head_layernorm, modeling_llama.LlamaRMSNorm)
    elif model_type == OPT_MODEL:
