...
class LlamaMLP(nn.Module):
    def __init__(self, config):
        ...
        self.gate_proj = RotLinear(
            self.hidden_size,
            self.intermediate_size,
            bias=False,
        )
        self.up_proj = RotLinear(
            self.hidden_size,
            self.intermediate_size,
            bias=False,
        )
        self.down_proj = RotLinear(
            self.intermediate_size,
            self.hidden_size,
            bias=False,
        )
...
class LlamaAttention(nn.Module):
    ...
    def __init__(
        ...
        self.q_proj = RotLinear(
            self.hidden_size,
            self.num_heads * self.head_dim,
            bias=config.attention_bias,
        )
        self.k_proj = RotLinear(
            self.hidden_size,
            self.num_key_value_heads * self.head_dim,
            bias=config.attention_bias,
        )
        self.v_proj = RotLinear(
            self.hidden_size,
            self.num_key_value_heads * self.head_dim,
            bias=config.attention_bias,
        )
        self.o_proj = RotLinear(
            self.hidden_size,
            self.hidden_size,
            bias=config.attention_bias,
        )
    ...
    def forward(
        ...
        if self.config.pretraining_tp > 1:
            ...
            query_states = self.q_proj(
                hidden_states, pre_rotation=rot_1 if not self.fused else None
            )
            ...
            value_states = self.v_proj(
                ...
                post_rotation=torch.kron(
                    ...
                    self.rot_2.t().contiguous() if not self.fused else None,
                ),
            )
        ...
        # Rotation #3 of key and query states to aid in quantization. Post ROPE but pre KV Cache
        query_states = torch.matmul(query_states, self.rot_3)
        key_states = torch.matmul(key_states, self.rot_3)
...
class LlamaDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        hidden_states = self.mlp(hidden_states, rot_1)
        hidden_states = residual + hidden_states
...
class LlamaPreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        # rot_init = self.config.rotation_initialization
        rot_init = "hadamard"
        if isinstance(module, nn.Linear):
            ...
            if rot_init == "identity":
                module.rot_2.data.copy_(
                    torch.eye(module.rot_2.data.shape[0], dtype=torch.float16)
                )
                module.rot_3.data.copy_(
                    torch.eye(module.rot_3.data.shape[0], dtype=torch.float16)
                )
                ...
                # module.rot_2.data.copy_(
                #     torch.randn(
                #         size=module.rot_2.data.shape, device=module.rot_2.device
                #     )
                # )
            ...
            if rot_init == "identity":
                module.rot_4.data.copy_(
                    torch.eye(module.rot_4.data.shape[0], dtype=torch.float16)
                )
            elif rot_init == "hadamard":
                module.rot_4.data.copy_(
                    random_hadamard_matrix(
                ...
                # module.rot_4.data.copy_(
                #     torch.randn(
                #         size=module.rot_4.data.shape, device=module.rot_4.device
                #     )
                # )
            ...
            if rot_init == "identity":
                ...
                # module.rot_1.data.copy_(
                #     torch.randn(
                #         size=module.rot_1.data.shape, device=module.rot_1.device
                #     )
                # )
