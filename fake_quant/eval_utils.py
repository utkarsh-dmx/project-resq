...
def evaluator(model, testenc, dev, args):
    ...
    for i in tqdm(range(len(layers)), desc="(Eval) Layers"):
        ...
        if args.capture_layer_io and args.layer_idx == i:
            captured_io = model_utils.capture_layer_io(
                model_utils.get_model_type(model), layer, inps
            )
    ...
    # for learnable rotation, the decoder layer outputs are rotated. So we need to undo it
    if args.rotate_mode == "learnable":
        for j in range(nbatches):
    ...
    if opt_type:
        if model.model.decoder.final_layer_norm is not None:
            model.model.decoder.final_layer_norm = (
                model.model.decoder.final_layer_norm.to(dev)
            )
